...
const Dissertations = db.dissertations;
...
exports.createResearchProposal = async (req, res) => {
    try {
      ...
      const proposal = await Dissertations.create({
        firstName,
        middleName,
        lastName,
        email,
...
exports.getAllResearchProposals = async (req, res) => {
  try {
    const proposals = await Dissertations.findAll({
  ...
  exports.updateResearchStatus = async (req, res) => {
    try {
      ...
      const [updatedCount] = await Dissertations.update(updateData, {
        where: { id },
      });
  
      if (updatedCount === 0) {
        return res.status(404).json({ message: "Dissertations not found or not updated" });
      }
  
      const updatedProposal = await Dissertations.findByPk(id);
